...
a,c.data(f,a)):null:(this.each(function(){c.data(this,a,b)}),l)},null,b,1<arguments.length,null,!0)},removedata:function(a){return this.each(function(){c.removedata(this,a)})}});c.extend({queue:function(a,b,d){var="" e;return="" a?(b="(b||"fx")+"queue",e=c._data(a,b),d&&(!e||c.isArray(d)?e=c._data(a,b,c.makeArray(d)):e.push(d)),e||[]):l},dequeue:function(a,b){b=b||"fx";var" d="c.queue(a,b),e=d.length,f=d.shift(),g=c._queueHooks(a,b),h=function(){c.dequeue(a,b)};"inprogress"===f&&(f=d.shift(),e--);(g.cur=f)&&" ("fx"="==b&&d.unshift("inprogress"),delete" g.stop,f.call(a,h,g));!e&&g&&g.empty.fire()},_queuehooks:function(a,b){var="" c._data(a,d)||c._data(a,d,{empty:c.callbacks("once="" memory").add(function(){c._removedata(a,b+"queue");c._removedata(a,d)})})}});c.fn.extend({queue:function(a,b){var="" a&&(b="a,a="fx",d--),d">arguments.length?c.queue(this[0],a):b===l?this:this.each(function(){var d=c.queue(this,a,b);c._queueHooks(this,a);"fx"===a&&"inprogress"!==d[0]&&c.dequeue(this,
...
"relative");var f=c(a),g=f.offset(),h=c.css(a,"top"),k=c.css(a,"left"),l={},p={},q,n;("absolute"===e||"fixed"===e)&&-1</n.timeout&&(k=settimeout(function(){v.abort("timeout")},n.timeout));try{o=1,s.send(y,d)}catch(t){if(!(2></arguments.length?this.on(b,null,a,c):this.trigger(b)}});c.fn.hover=function(a,b){return></arguments.length)},show:function(){return></e.length&&ya(e,!m&&c(a,"script")),g},buildfragment:function(a,b,d,e){for(var></c.filter(a,[d]).length)&&(b||1!==d.nodetype||c.cleandata(c(d)),d.parentnode&&(b&&c.contains(d.ownerdocument,d)&&ya(c(d,"script")),d.parentnode.removechild(d)));return></([\w:]+)></h.index(d):c.find.matchesselector(d,a)){g.push(d);break}d=d.parentnode}return></=c(a,this.context).index(this[0]):0<c.filter(a,this).length:0<this.filter(a).length)},closest:function(a,b){for(var></f?c.unique(d):d),d.selector=(this.selector?this.selector+"></=--b;)a.push(b);return></h(b,n,null,[a]).length};h.contains=function(a,b){return(a.ownerdocument||a)!==n&&ma(a),sa(a,b)};h.attr=function(a,b){var></b.length,e=0<a.length,g=function(f,g,k,m,l){var></g&&fa(m),1<g&&p(a.slice(0,g-1)).replace(q,"$1"),c,d></(l=e?s.call(f,n):s[k])&&(f[l]=!(g[l]=n))}}else></l.length&&"id"===(s=l[0]).type&&9===b.nodetype&&!h&&t.relative[l[1].type]){if(b=t.find.id(s.matches[0].replace(aa,ba),b)[0],!b){l=c;break></=c(d,this).index(m):c.find(d,this,null,[m]).length),></s?k:m.bindtype||n,(f=(c._data(h,"events")||{})[a.type]&&c._data(h,"handle"))&&f.apply(h,b),(f=g&&h[g])&&c.acceptdata(h)&&f.apply&&!1===f.apply(h,b)&&a.preventdefault();if(a.type=n,!(e||a.isdefaultprevented()||m._default&&!1!==m._default.apply(d.ownerdocument,b)||"click"===n&&c.nodename(d,"a"))&&c.acceptdata(d)&&g&&d[n]&&!c.iswindow(d)){(f=d[g])&&(d[g]=null);c.event.triggered=n;try{d[n]()}catch(w){}c.event.triggered=l;f&&(d[g]=f)}return></=c.inarray(c(this).val(),d)}),d.length||(a.selectedindex=-1),d}}},attr:function(a,b,d){var></=("></=e.indexof("></arguments.length)},removeattr:function(a){return></arguments.length,null,!0)},removedata:function(a){return></c.inarray(a,k):!(!k||!k.length)},></(e=c.inarray(d,k,e));)k.splice(e,1),b&&(f></--c.readywait||(ka.resolvewith(p,[c]),c.fn.trigger&&c(p).trigger("ready").off("ready"))}},isfunction:function(a){return"function"===c.type(a)},isarray:array.isarray||function(a){return"array"===c.type(a)},iswindow:function(a){return></=a&&b></=></\></(\w+)\s*\></[\w\w]+></=c.inarray(a,b)===d})}function></b&&b-1>